...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Content.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Content.attributes).reduce((acc, curr) => {
      switch (Content.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
